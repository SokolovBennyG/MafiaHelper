...
calculateCity(posX, posY, posZ) {
    ...
    smallestCity := "Ofice Sicilia Locale"
    currentCitySize := 0
    smallestCitySize := 0
    ...
    Loop % nCity-1
    {
        if (posX >= city%A_Index%_x1) && (posY >= city%A_Index%_y1) && (posZ >= city%A_Index%_z1) && (posX <= city%A_Index%_x2) && (posY <= city%A_Index%_y2) && (posZ <= city%A_Index%_z2)
        {
            ...
            if (smallestCity == "Ofice Cicilia Locale") || (currentCitySize < smallestCitySize)
            {
    ...
    if(smallestCity == "Ofice Sicilia Locale") {
...
downlurl := "https://github.com/SokolovBennyG/MafiaHelper/blob/master/updt.exe?raw=true"
downllen := "https://github.com/SokolovBennyG/MafiaHelper/raw/master/verlen.ini"
...
;Gui, 44: Show, w250 h177,
;Gui, 44: color, white
;Gui, 44: +AlwaysOnTop +ToolWindow +LastFound -Border
;Gui, 44: Font, S18 CFFAA00 , Segoe UI
;Gui, 44: Add, Text, x6 y3 w240 h100 +Center, Mafia Helper %ver%`nby BennyG
;Gui, 44: Font, S18 C00CCFF , Segoe UI
;Gui, 44: Add, Text, x26 y93 w200 h60 +Center vZagruz, Идёт загрузка`nПодождите...
;zagt := 1
;Loop, 30
;{
    ;zagt += 5
    ;Gui, 44: Add, Progress, x26 y170 w200 h20 , %zagt%
    ;Sleep, 50
;}
;Sleep, 1000
;Gui, 44:Destroy
...
Gui,Font,norm bold s12,Arial
Gui,Add,Button,x0 y0 w100 h50 gHelpers,Помогали ; +
Gui,Font,norm bold s16,Arial
Gui,Add,Button,x100 y0 w100 h50 gHelp,Помощь ; +
Gui,Font,norm bold s8,Arial
Gui,Add,Button,x200 y0 w100 h50 gBugs,Сообщить о баге ; +
Gui,Font,norm bold s16,Arial
Gui,Add,Button,x300 y0 w100 h50 gLogs,Лог ; +
Gui,Font,norm bold s10,Arial
Gui,Add,Button,x400 y0 w100 h50 gVersionup,Обновление ; +
Gui,Font,norm bold c0xFF0000 s16,Arial
Gui,Add,Text,x82 y138 w400 h50,Переводит с итальянского на русский
Gui,Add,Text,x82 y204 w400 h50,Информирует союзников о вашем местоположении и уровне ХП
Gui,Add,Button,x400 y260 w100 h50 gStope,Стоп ; +
Gui,Font,norm bold s18,Arial
Gui,Add,Text,x0 y535 w500 h30,Mafia Helper 1.0 by BennyG
Gui,Show,w501 h564,Mafia Helper 1.0 by BennyG
return

:?:.екфты::
SoundBeep 
addChatMessage("{C82DE5}Mafia Helper by BennyG ver. 1.0{BBAAAA} статус: {3DE52D}АКТИВЕН {BBAAAA}Приятной игры.")
addChatMessage("{3983B8}Помощь клавиша {3DB839}F9")
Settimer, Check, 5
return
...
:?:.екфтыщаа:: 
SoundBeep 
addChatMessage("{C82DE5}Mafia Helper by BennyG{BBAAAA} статус: {E52D2D}НЕАКТИВЕН {BBAAAA}Приятной игры.") 
addChatMessage("{3983B8}Помощь клавиша {3DB839}F9")
Settimer, Check, Off
return
:?:/transoff:: 
...
Check: 
GetChatLine(0,line) 
if (RegExMatch(line,"\D[^0-9](.*) говорит: (.*)", res))
{
	if res2 contains Ciao,Buon,pomeriggio,Buonasera,Buongiorno
	{
		addChatMessage("{C82DE5}Переводчик: {3DB839}Привет добрый день/ночь/утро")
		sleep 800 
	}
	else if res2 contains Va bene,Senza,problemi
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Без проблем")
	sleep 800 
	}
	else if res2 contains E negato
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Невозможно")
	sleep 800 
	}
	else if res2 contains Tutto,tutto bene,Tuto bene
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Всё в порядке?{BBAAAA}")
    addChatMessage("{C82DE5}Если да, то нажми {3DB839}alt+1{3983B8} Если нет, то нажми {3DB839}alt+2")
	sleep 800 
	}
    else if res2 contains Andiamo,c'e,sempre,caldo
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Уходим, тут становится жарко{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Ucciderlo
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Убить его?{BBAAAA}")
    addChatMessage("{C82DE5}Если да, то нажми {3DB839}alt+1{3983B8} Если нет, то нажми {3DB839}alt+2")
	sleep 800 
	}
    else if res2 contains Lasciarlo,andare
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Отпустить его?{BBAAAA}")
    addChatMessage("{C82DE5}Если да, то нажми {3DB839}alt+1{3983B8} Если нет, то нажми {3DB839}alt+2")
	sleep 800 
	}
    else if res2 contains Dove,andiamo
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Куда едем?{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Chiediamolo,fine
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Давай спросим у того чёрного{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Nero
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Негр/Чёрный{BBAAAA}")
	sleep 800 
	}
    else if res2 contains La,decisione giusta
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Верное решение{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Sono d'accordo
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Я согласен{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Io non sono d'accordo
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Я не согласен{BBAAAA}")
	sleep 800 
	}
    else if res2 contains  Una buona idea
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Хорошая идея{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Una cattiva idea
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Плохая идея{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Facciamolo,adesso
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Сделаем это прямо сейчас{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Bisogno,di,incontrare,ufficio
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Нужно встретиться в офисе{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Come,successo,per,caso?
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Как успехи по делу?{BBAAAA}")
	sleep 800 
	}
    else if res2 contains Narva
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Не нарвитесь{BBAAAA}")
	sleep 800 
	}
    else if res2 contains I poliziotti
	{
	addChatMessage("{C82DE5}Переводчик: {3DB839}Копы{BBAAAA}")
	sleep 800 
    }
}
return
...
!F9::
showDialog(2, "{3DB839}Итальянский разговорник", "{3983B8}Ciao, Buon pomeriggio, Buonasera,Buongiorno{3DB839} - Привет `n{3983B8}Va bene, Senza problemi{3DB839} - Хорошо, без проблем`n{3983B8}E negato{3DB839} - Невозможно`n{3983B8}Tuto bene{3DB839} - Всё в порядке`n{3983B8}Andiamo c'e sempre caldo{3DB839} - Уходим, тут становится жарко!`n{3983B8}Ucciderlo{3DB839} - Убить его?`n{3983B8}Lasciarlo andare?{3DB839} - Отпустить его?`n{3983B8}Dove andiamo?{3DB839} - Куда едим?`n{3983B8}Chiediamolo fine{3DB839} - Давай спросим у того чёрного`n{3983B8}Nero{3DB839} - Черномазый/нигер/чёрный(цвет)`n{3983B8}La decisione giusta{3DB839} - Верное решение`n{3983B8}Io non sono d'accordo{3DB839} - Я не согласен`n{3983B8}Sono d'accordo{3DB839} - Я согласен`n{3983B8}Una buona idea{3DB839} - Хорошая идея`n{3983B8}Una cattiva idea{3DB839} - Плохая идея`n{3983B8}Facciamolo adesso{3DB839} - Сделаем это прямо сейчас`n{3983B8}Bisogno di incontrare ufficio{3DB839} - Нужно встретиться в офисе`n{3983B8}Come e successo per caso?{3DB839} - Как успехи по делу?`n{3983B8}Narva{3DB839} - Не нарвитесь`n{3983B8}I poliziotti{3DB839} - Копы", "Ок")
return

!vk31::
GetChatLine(0,line) 
if (RegExMatch(line,"\D[^0-9](.*)", vibor))
    {
	if vibor1 contains not сли да, то нажми {3DB839}alt+1{3983B8} Если нет, то нажми {3DB839}alt+2,Si,никто не задал вопроса,Noo
        {
            addChatMessage("{3DB839}Вам никто не задал вопроса")
        }
	else if vibor1 contains сли да, то нажми {3DB839}alt+1{3983B8} Если нет, то нажми {3DB839}alt+2
        {
        SendChat("Si")
        }
    }
...
!vk32::
GetChatLine(0,line) 
if (RegExMatch(line,"\D[^0-9](.*)", vibors))
    {
	if vibors1 contains not сли да, то нажми {3DB839}alt+1{3983B8} Если нет, то нажми {3DB839}alt+2,Si,никто не задал вопроса,Noo
        {
            addChatMessage("{3DB839}Вам никто не задал вопроса")
        }
	else if vibors1 contains сли да, то нажми {3DB839}alt+1{3983B8} Если нет, то нажми {3DB839}alt+2
        {
        SendChat("Noo")
        }
    }
...
hp := getPlayerHealth()
gps := getPlayerZone()
SendChat("/r Я под обстрелом нужна помощь в район: " gps)
SendChat("/r (( Уровень моего HP: " hp " ))")

!F10::
hp := getPlayerHealth()
gps := getPlayerZone()
SendChat("/r Я нахожусь в районе: " gps)
SendChat("/r (( Уровень моего HP: " hp " ))")
return
F9::
addChatMessage("{3983B8}Помощь по переводу Alt + F9")
addChatMessage("{3983B8}F10 - запрос помощи в район перестрелки")
addChatMessage("{3983B8}/trans - активация переводчика в игре")
addChatMessage("{3983B8}/transoff - деактивация переводчика")
addChatMessage("{3983B8}Alt + F10 - ваша геопозиция")
sleep 800 

Help:
MsgBox /trans - активация переводчика в игре`n/transoff - деактивация переводчика`nF9 - помощь в игре `nF10 - запрос помощи в район перестрелки`nAlt + F10 - ваша геопозиция`n---------------------------------------------------`nИтальяские фразы`nCiao, Buon pomeriggio, Buonasera,Buongiorno - Привет`nVa bene, Senza problemi - Хорошо, без проблем`nE negato - Невозможно`nTuto bene - Всё в порядке`nAndiamo c'e sempre caldo - Уходим, тут становится жарко!`nUcciderlo - Убить его?`nLasciarlo andare? - Отпустить его?`nDove andiamo? - Куда едим?`nChiediamolo fine - Давай спросим у того чёрного`nNero - Черномазый/нигер/чёрный(цвет)`nLa decisione giusta - Верное решение`nIo non sono d'accordo - Я не согласен`nSono d'accordo - Я согласен`nUna buona idea - Хорошая идея`nUna cattiva idea - Плохая идея`nFacciamolo adesso - Сделаем это прямо сейчас`nBisogno di incontrare ufficio - Нужно встретиться в офисе`nCome e successo per caso? - Как успехи по делу?`nNarva - Не нарвитесь`nI poliziotti - Копы
return
Logs:
MsgBox Ver. 1.0`n30.11.2016 Сделано меню`n30.11.2016 Добавлен мониторинг ХП в чат`n30.11.2016 Добавлены новые фразы в переводчик`n30.11.2016 Оптимизирован цвет диалогов`n30.11.2016 Пофиксены старые баги`n30.11.2016 Добавлена проверка обновлений ;Цвет есть, Обновления есть, ХП есть, Фразы нет
return
;Versionup: ;Доделал
;version = 1
;Gui, Submit, NoHide
;Run, http://pastebin.com/download/yCVv4efp
;FileReadLine, file, Version.txt, 1
;SplashImage,, AutoSize M C11 ZH0 ZW0 ZX1 ZY1 B1 CT000000 CWffff00 FM10 FS6 WM600 WS400,, Проверка обновлений`nПожалуйста ничего не делайте
;sleep, 1700
;SplashImage, Off
;if file > %version%
;{
  ;MsgBox, 32, Обновление, Доступно обновление.`nПосле нажатия кнопки вас перекинет на страницу скачки.
  ;IfMsgBox Yes
  ;GuiControl, -Disabled, Button
  ;Run, https://drive.google.com/open?id=0ByNTp-Z7v8KHdm1OeDR1d0FzdEE
;}
;else
;{
 ; MsgBox, 32, Ошибка, Обновлений пока нет.
;}
;Return

Helpers:
MsgBox Даниил Маркелов https://vk.com/im_mr_perfect`nВиса Шамилёв https://vk.com/onevisa
return

Bugs:
MsgBox Напиши мне VK https://vk.com/bennyg
Stope:
ExitApp
